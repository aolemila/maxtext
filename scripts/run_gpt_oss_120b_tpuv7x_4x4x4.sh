##########################COPY FROM########## 
# https://github.com/AI-Hypercomputer/tpu-recipes/blob/main/training/ironwood/gpt-oss-120b/8k-bf16-tpu7x-4x4x4/xpk/run_recipe.sh
#############################################
#!/bin/bash
...
if [ -n "${FALCON_ARTIFACT_URI:-}" ]; then
  export BASE_OUTPUT_DIR="${FALCON_ARTIFACT_URI%/}"
else
  export BASE_OUTPUT_DIR="gs://inference-model-storage-poc-tpu-hns/runs"
fi
...
XLA_FLAGS="--xla_dump_to=/gcs/google_support_reproduce/hang_with_custom_call_region_trace/hlo_dump/rank${FALCON_RANK} --xla_dump_hlo_as_text"
